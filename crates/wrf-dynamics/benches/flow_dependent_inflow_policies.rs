//! Criterion benchmark for WRF flow-dependent scalar inflow policies.
#![allow(missing_docs)]
...
use wrf_dynamics::{
    ...
    SpecifiedBoundaryFlowRegion, SpecifiedBoundaryInflowPolicy,
...
fn benchmark(criterion: &mut Criterion) {
    ...
    let mut group = criterion.benchmark_group("flow_dependent_inflow_policies");
    ...
    for workers in worker_counts() {
        for (name, policy) in policies() {
            ...
            group.bench_with_input(BenchmarkId::new(name, workers), &workers, |bencher, _| {
                bencher.iter(|| fixture.apply(black_box(&backend), black_box(policy)));
            });
        }
    }
    group.finish();
...
fn report_settled_allocations() {
    ...
    for (name, policy) in policies() {
        let backend = CpuBackend::try_with_worker_count(workers).unwrap();
        let mut fixture = Fixture::new(&backend);
        fixture.apply(&backend, policy);
        let allocation_region = Region::new(GLOBAL_ALLOCATOR);
        for _ in 0..100 {
            fixture.apply(&backend, policy);
        }
        eprintln!(
            "{name} settled allocations per 100 calls: {:?}",
            allocation_region.change()
        );
    }
}

fn policies() -> [(&'static str, SpecifiedBoundaryInflowPolicy); 2] {
    [
        ("constant", SpecifiedBoundaryInflowPolicy::Constant(73.5)),
        ("preserve", SpecifiedBoundaryInflowPolicy::Preserve),
    ]
}
...
impl Fixture {
    ...
    fn apply(&mut self, backend: &CpuBackend, inflow_policy: SpecifiedBoundaryInflowPolicy) {
        backend
            .apply_flow_dependent_specified_boundary(
                ...
                SpecifiedBoundaryFlowParameters::with_inflow_policy(5, inflow_policy),
