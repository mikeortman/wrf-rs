...
pub struct AcousticTrajectoryCoefficients<'a> {
    ...
    pub(crate) upper_full_level_weight: &'a [f32],
    pub(crate) lower_full_level_weight: &'a [f32],
}

impl<'a> AcousticTrajectoryCoefficients<'a> {
    /// Groups the ten live one-dimensional coefficient roles.
    ///
    /// `upper_full_level_weight` is WRF `fnm`; `lower_full_level_weight`
    /// is WRF `fnp`. The physical names avoid relying on legacy kernel APIs
    /// whose lower/upper labels are not consistent with one another.
    ...
    pub const fn new(
        ...
        upper_full_level_weight: &'a [f32],
        lower_full_level_weight: &'a [f32],
    ) -> Self {
        Self {
