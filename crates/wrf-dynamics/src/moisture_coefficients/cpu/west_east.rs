...
pub(super) fn calculate(
    ...
    let (west_east, south_north, bottom_top) = region.west_east_output_ranges();
                    ...
                    if moisture.is_empty() {
                        ...
                        for species in moisture.active {
                            ...
                            for (offset, total) in totals.iter_mut().enumerate() {
                                let current_index = row_start + west_east.start + offset;
                                *total =
                                    (*total + values[current_index]) + values[current_index - 1];
                            }
                        }
                        for total in totals {
