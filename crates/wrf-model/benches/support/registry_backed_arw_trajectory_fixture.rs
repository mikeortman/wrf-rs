...
/// Representative 64 x 64 x 40 common-storage accepted trajectory fixture.
pub struct RegistryBackedArwTrajectoryFixture {
    /// Registry-bound accepted-stage model.
    pub model: RegistryBoundArwModel,
    /// Restart-owned state reused across measurements.
    pub state: ArwModelState,
    /// Diagnostic and tendency storage reused across measurements.
    pub workspace: ArwModelWorkspace,
    binding: ArwRegistryBinding,
}

impl RegistryBackedArwTrajectoryFixture {
    /// Allocates the fixture once and initializes all numerical storage.
    pub fn new(backend: &CpuBackend) -> Self {
        ...
        .unwrap();
        let geometry = ArwModelGeometry::try_new(62, 62, 38).unwrap();
        ...
        let model =
            RegistryBoundArwModel::try_kessler(binding.clone(), geometry, coefficients, controls())
                .unwrap();
        let state = model.create_state(backend).unwrap();
        let workspace = model.create_workspace(backend).unwrap();
    ...
    /// Restores the deterministic initial condition without allocating.
    pub fn reset(&mut self) {
        initialize_state(&self.binding, &mut self.state);
...
fn initialize_state(binding: &ArwRegistryBinding, state: &mut ArwModelState) {
    ...
    for south_north in 0..shape.south_north_points() {
        for bottom_top in 0..shape.bottom_top_points() {
            for west_east in 0..shape.west_east_points() {
                ...
                for field in [
                    ArwMassField::PreviousWestEastVelocity,
                    ArwMassField::CurrentWestEastVelocity,
                ] {
                    state.mass_field_values_mut(field)[index] = u;
                }
                for field in [
                    ArwMassField::PreviousSouthNorthVelocity,
                    ArwMassField::CurrentSouthNorthVelocity,
                ] {
                    state.mass_field_values_mut(field)[index] = v;
                }
                for field in [
                    ArwMassField::PreviousVerticalVelocity,
                    ArwMassField::CurrentVerticalVelocity,
                ] {
                    state.mass_field_values_mut(field)[index] = w;
                }
                for field in [
                    ArwMassField::PreviousPotentialTemperature,
                    ArwMassField::CurrentPotentialTemperature,
                ] {
                    state.mass_field_values_mut(field)[index] = t;
                }
    ...
    for south_north in 0..w_shape.south_north_points() {
        for bottom_top in 0..w_shape.bottom_top_points() {
            for west_east in 0..w_shape.west_east_points() {
                ...
                for field in [
                    ArwGeopotentialField::PreviousPerturbation,
                    ArwGeopotentialField::CurrentPerturbation,
                ] {
                    state.geopotential_field_values_mut(field)[index] = ph;
                }
    ...
    for south_north in 0..horizontal.south_north_points() {
        for west_east in 0..horizontal.west_east_points() {
            ...
            for field in [
                ArwColumnField::PreviousPerturbationMass,
                ArwColumnField::CurrentPerturbationMass,
            ] {
                state.column_field_values_mut(field)[index] = mu;
            }
