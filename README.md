...
- `wrf-dynamics` contains line-parallel, scratch-free ports of WRF's
  ...
  dry-air omega diagnosis. Deterministic fixtures and seeded randomized corpora
  check upstream `REAL` bit patterns.
...
| ARW dynamical core | In progress | Positive-definite sheet/slab, Held-Suarez damping, all three column-mass staggering entry points, `couple_momentum`, and `calc_ww_cp` have exact Fortran oracles, matched optimized-Fortran benchmarks, CPU scaling results, and allocation budgets | Continue `rk_step_prep` with `calc_cq` and dependency-closed integration |
