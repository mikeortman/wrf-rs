...
expected="$repository_root/crates/wrf-dynamics/test-data/specified_boundary_update.out.correct"

command -v gfortran >/dev/null 2>&1 || {
    echo "missing command required by specified-boundary oracle: gfortran" >&2
    exit 1
}
build_directory=$(mktemp -d "${TMPDIR:-/tmp}/wrf-rs-specified-boundary.XXXXXX")
...
gfortran -O0 -ffp-contract=off -ffree-form -ffree-line-length-none \
    ...
    "$parity_directory/specified_boundary_update_driver.F90" \
...
echo "PASS WRF spec_bdyupdate oracle"
