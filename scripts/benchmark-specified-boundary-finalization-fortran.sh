...
build_directory=$(mktemp -d "${TMPDIR:-/tmp}/wrf-rs-specified-boundary-finalization-benchmark.XXXXXX")
...
gfortran -O3 -flto -ffp-contract=off -ffree-form -ffree-line-length-none \
    ...
    "$parity_directory/specified_boundary_finalization_benchmark.F90" \
