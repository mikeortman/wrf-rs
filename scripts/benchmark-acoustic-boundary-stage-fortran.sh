...
build_directory=$(mktemp -d "${TMPDIR:-/tmp}/wrf-rs-acoustic-boundary-stage-benchmark.XXXXXX")
trap 'rm -rf "$build_directory"' EXIT HUP INT TERM

command -v gfortran >/dev/null || {
  echo "missing command required by acoustic-boundary-stage benchmark: gfortran" >&2
  exit 1
}
{
...
gfortran -O3 -flto -ffp-contract=off -fno-range-check -fallow-argument-mismatch \
  -cpp -ffree-form -ffree-line-length-none \
  "$parity_directory/module_configure_stub.F90" "$build_directory/extracted.F90" \
  "$parity_directory/acoustic_boundary_stage_benchmark.F90" -o "$build_directory/benchmark"
