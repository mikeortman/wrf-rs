...
expected="$repository_root/crates/wrf-dynamics/test-data/flow_dependent_inflow_policies.out.correct"

command -v gfortran >/dev/null 2>&1 || {
    echo "missing command required by flow-dependent inflow-policy oracle: gfortran" >&2
    exit 1
}
build_directory=$(mktemp -d "${TMPDIR:-/tmp}/wrf-rs-flow-dependent-inflow-policies.XXXXXX")
...
gfortran -O0 -ffp-contract=off -ffree-form -ffree-line-length-none \
    ...
    "$parity_directory/flow_dependent_inflow_policies_driver.F90" \
...
echo "PASS WRF flow-dependent inflow-policy oracle"
