...
| Domain decomposition and halo exchange | Pinned `task_for_point.c` assigns all patches exactly for three centered-remainder grids; pinned `period.c` and `f_pack.F90` match every selected periodic destination and doubly periodic corner | signed half-open conversion, non-one origins, WRF guard-point memory bounds, invalid layouts before construction, clipped edge tiles, internal edges/corners, field staggering, bounded message buffers, complete local-vs-four-rank MPI memory equality | direct `set_tiles2` oracle; generated asymmetric halo descriptors; one-dimensional and larger process grids under MPI; nested/intermediate domains; multi-field message aggregation; operational model fields |
