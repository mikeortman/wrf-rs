...
| `calc_mu_staggered` interior interpolation | Exact routine body extracted from the pinned large module and compiled; 60 raw-bit output/sentinel values | typed planar/range/preceding-neighbor validation, shape failure before mutation, one-vs-four-worker determinism, untouched halos | four physical-boundary branches; periodic `calc_mu_uv` variants; randomized corpus; matched Fortran benchmark and allocation budget |
